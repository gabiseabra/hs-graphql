...
module GraphQL.Parser.Parser where
...
import           GraphQL.Parser.Lexer           ( (<@>)
                                                , Parser
                                                )
import qualified GraphQL.Parser.Lexer          as L
...
typeDefP = label "VariableDefinition" $ L.lexeme $ Fix <$> do
  var      <- choice [ListType <$> L.brackets typeDefP, NamedType <$> L.name]
  required <- isJust <$> optional (char '!')
  if required then pure . NonNullType . Fix $ var else pure var
...
valueP =
  label "Value"
    $   choice [InL . Const <$> L.varName, InR <$> constValueFP valueP]
    <@> (:<)
...
fieldP ty =
  label "Field" $ Field ty <$> aliasP <*> L.name <*> L.optional_ argsP

selectionNodeP :: Maybe Name -> Parser (Selection (Field Value))
selectionNodeP ty = label "Selection"
  $ choice [try inlineFragment, fragmentSpread, node]
 where
  node = do
    pos   <- L.getPos
    field <- fieldP ty
...
validateDocumentP (frags, ops) =
  Document <$> validateFragmentsP frags <*> validateOperationsP ops
...
validateOperationsP ops =
  InR
    <$> (sequence . HashMap.fromListWithKey (liftJoin2 . onDupe) =<< mapM go ops
        )
 where
  go op
    | Just name <- opName op = pure (name, pure op)
    | otherwise = L.validationErrorP
      [opPos op]
      "Unnamed operation in document with multiple operations"
  onDupe k op0 op1 =
    L.validationErrorP [opPos op0, opPos op1]
...
validateFragmentsP =
  sequence . HashMap.fromListWithKey (liftJoin2 . onDupe) . fmap
    (fragName &&& pure)
 where
  onDupe k frag0 frag1 =
    L.validationErrorP [fragPos frag0, fragPos frag1]
      $  "Duplicated fragment name "
      <> k
