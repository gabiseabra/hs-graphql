...
import           GraphQL.Schema                 ( OperationType(..) )
...
instance Show1 ConstValueF where
  ...
  liftShowsPrec sp sl d (ListVal as) =
    showsUnaryWith (liftShowsPrec sp sl) "ListVal" d as
  liftShowsPrec sp sl d (ObjectVal as) =
    showsUnaryWith (liftShowsPrec sp sl) "ObjectVal" d as
...
checkTypeDefinition (Fix (ListType ty)) (_ :< ListVal as) =
  getAll . foldMap (All . checkTypeDefinition ty) $ as
...
instance Eq1 Field where
  liftEq f (Field ty alias name args) (Field ty' alias' name' args') =
    ty == ty' && alias == alias' && name == name' && liftEq f args args'

instance Show1 Field where
  liftShowsPrec sp sl d (Field ty alias name args) = showsUnaryWith
    (liftShowsPrec sp sl)
...
instance Show a => Show1 (SelectionF a) where
  liftShowsPrec sp sl d (Node a r) =
    showsUnaryWith (liftShowsPrec sp sl) ("Node " <> show a) d r
  liftShowsPrec sp sl d (FragmentSpread a) = (<>) $ "FragmentSpread " <> show a
  liftShowsPrec sp sl d (InlineFragment a r) =
    showsUnaryWith (liftShowsPrec sp sl) ("InlineFragment " <> show a) d r
...
_nodes
  :: IndexedTraversal
       [Ix a]
       (Cofree (SelectionF a) x)
       (Cofree (SelectionF b) x)
       a
       b
_nodes = flip go []
 where
  go f path (x :< y) = (x :<) <$> case y of
    Node a xs ->
      Node <$> indexed f path a <*> itraverse (go f . (: path) . NodeIx a) xs
    InlineFragment a xs ->
      InlineFragment a <$> itraverse (go f . (: path) . FragmentIx a) xs
...
_fragSelection f frag =
  f (fragSelection frag) <&> \a -> frag { fragSelection = a }

instance Eq1 Fragment where
  liftEq f (Fragment pos name ty as) (Fragment pos' name' ty' bs) =
    pos == pos' && name == name' && ty == ty' && liftEq f as bs

instance Show1 Fragment where
  liftShowsPrec sp sl d (Fragment pos name ty as) = showsUnaryWith
    (liftShowsPrec sp sl)
...
instance Eq1 Operation where
  liftEq f (Query pos name vars as) (Query pos' name' vars' bs) =
    pos == pos' && name == name' && vars == vars && liftEq f as bs
  liftEq f (Mutation pos name vars as) (Mutation pos' name' vars' bs) =
    pos == pos' && name == name' && vars == vars && liftEq f as bs
  liftEq f (Subscription pos name vars a) (Subscription pos' name' vars' b) =
    pos == pos' && name == name' && vars == vars && liftEq f
                                                           (Identity a)
                                                           (Identity b)
  liftEq _ _ _ = False

instance Show1 Operation where
  liftShowsPrec sp sl d (Query pos name vars as) = showsUnaryWith
    (liftShowsPrec sp sl)
    ("Query " <> show pos <> " " <> show name <> " " <> show vars)
    d
    as
  liftShowsPrec sp sl d (Mutation pos name vars as) = showsUnaryWith
    (liftShowsPrec sp sl)
    ("Mutation " <> show pos <> " " <> show name <> " " <> show vars)
    d
    as
  liftShowsPrec sp sl d (Subscription pos name vars a) = showsUnaryWith
    sp
    ("Subscription " <> show pos <> " " <> show name <> " " <> show vars)
    d
    a
...
_docOperations
  :: Lens' (Document a) ((Identity :+: HashMap Name) (Operation a))
