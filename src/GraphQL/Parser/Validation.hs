...
import           GraphQL.Response               ( ErrorCode(..)
                                                , GraphQLError(..)
                                                , Pos
                                                )
...
type Rule a b
  = (  forall m
     . MonadError (NonEmpty GraphQLError) m
    => Document a
    -> m (Document b)
    )
...
basicRules = validateDefaultValues <=< unfoldFields
...
mergeOverlappingFields = traverse . cataM $ \(pos CofreeT.:< NodeF a as) ->
  (pos :<) . NodeF a <$> foldM merge [] as
 where
  merge as a = case List.find (on overlaps (node . unwrap) a) as of
    Nothing                       -> pure (a : as)
    Just b | forget a == forget b -> pure as
    Just b                        -> throwError . pure $ GraphQLError
      VALIDATION_ERROR
      (Just [extract a, extract b])
      Nothing
      "Conflicting overlapping fields"
...
validateDefaultValues doc = do
  ...
  pure doc
 where
  printValue = LazyText.toStrict . JSON.encodeToLazyText . cata
    (JSON.toJSON . CofreeT.tailF)
  ...
  validation var@(Variable _ def (Just (pos :< val))) =
    if checkTypeDefinition def (pos :< val)
      then pure ()
        ...
        throwError
        .  pure
        $  GraphQLError VALIDATION_ERROR (Just [pos]) Nothing
        $  "Expected "
        <> Text.pack (show def)
        <> ", found "
        <> printValue (pos :< val)
...
unfoldFields doc = do
  ...
  let unused = HashMap.filterWithKey (\k _ -> Set.notMember k visited)
        $ fragments doc'
  if length visited == length (fragments doc)
    then pure doc'
      ...
      throwError
      .  pure
      $  GraphQLError VALIDATION_ERROR
                      (Just (fmap fragPos . HashMap.elems $ unused))
                      Nothing
...
unfoldFieldsST
  :: MonadError (NonEmpty GraphQLError) m
  => Document (Selection a)
  -> StateT (Set Name) m (Document (Tree a))
unfoldFieldsST doc = do
 ...
 where
  traverseFrag frags = traverseOf (traverse . _fragSelection)
    $ traverseAccum (eraseFragments $ visitFragment frags)
  traverseOp frags = traverseOf (traverse . _opSelection)
    $ traverseAccum (eraseFragments $ pickFragment frags)
...
eraseFragments
  ...
  -> Selection a
  -> f (NonEmpty (Tree a))
eraseFragments f (pos :< Node a as) =
  pure
    .   (pos :<)
    .   NodeF a
    <$> traverseAccum (fmap NE.toList . eraseFragments f) as
eraseFragments f (_ :< InlineFragment _ as) =
  traverseAccum (eraseFragments f) as
eraseFragments f (pos :< FragmentSpread k) = f pos k

pickFragment
  :: MonadError (NonEmpty GraphQLError) m
  => HashMap Name (Fragment a)
  -> Pos
  -> Name
  -> StateT (Set Name) m (NonEmpty a)
...
visitFragment
  :: MonadError (NonEmpty GraphQLError) m
  => HashMap Name (Fragment (Selection a))
  -> Pos
  -> Name
  -> StateT (Set Name) m (NonEmpty (Tree a))
...
traverseAccum
  :: (Monad m, Traversable f, Monad f) => (a -> m (f b)) -> f a -> m (f b)
