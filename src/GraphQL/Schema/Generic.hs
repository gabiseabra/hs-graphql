...
module GraphQL.Schema.Generic where
...
scalarDef
  :: forall a
   . JSON.FromJSON a
  => JSON.ToJSON a => Typename -> TypeDef SCALAR a
scalarDef ty = ScalarType ty Nothing

enumDef
  :: forall a
   . GR.Generic a
  => GraphQLEnumG (GR.Rep a) => Typename -> TypeDef ENUM a
enumDef ty = EnumType ty Nothing enumValues encodeEnum
 where
  enumValues =
    Map.fromList . (fmap . fmap . fmap) GR.to . enumValues' $ Proxy @(GR.Rep a)
...
instance (GraphQLEnumG l, GraphQLEnumG r) => GraphQLEnumG (l GR.:+: r) where
  enumValues' _ =
    (fmap . fmap . fmap) GR.L1 (enumValues' (Proxy @l))
      <> (fmap . fmap . fmap) GR.R1 (enumValues' (Proxy @r))
...
unionDef ty =
  UnionType ty Nothing
    $ Map.fromList
    $ eraseF @(GraphQLObjectType m) @(Kleisli Maybe a) @(Var.NativeRow a) go
    $ variantCases @(GraphQLObjectType m) @a
 where
  go
    :: forall b
     . GraphQLObjectType m b
    => Kleisli Maybe a b
    -> (Text, Variant m a)
...
resolverDef ty =
 ...
 where
  ...
  go = resolver mempty . flip . fmap mkFieldResolver
...
resolver
  ...
  => GraphQLInput i => Maybe Text -> (i -> a -> m r) -> Resolver m a
resolver desc = Exists2 . Field desc . fmap Kleisli
